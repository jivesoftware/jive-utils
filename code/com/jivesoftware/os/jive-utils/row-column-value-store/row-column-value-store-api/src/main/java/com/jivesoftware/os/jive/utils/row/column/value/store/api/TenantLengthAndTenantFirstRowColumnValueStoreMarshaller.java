...
/**
 * Marshaller for an HBase table that stores the tenant length and tenant ID at the front. The tenantId and row key are converted to lexicographic sortable
 * bytes and combined into a single byte array with the tenantId at the front. The column keys are also converted to lexicographic sortable bytes. Values
 * are converted to just normal bytes, not necessarily lexicographic sortable.
 *
 * Specifically, the row key bytes will end up looking as follows:
 *
 * [length of tenantId bytes][tenantId bytes][row key object bytes]
 *
 ...
 */
public class TenantLengthAndTenantFirstRowColumnValueStoreMarshaller<T, R, C, V> implements RowColumnValueStoreMarshaller<T, R, C, V> {
    ...
    public TenantLengthAndTenantFirstRowColumnValueStoreMarshaller(TypeMarshaller<T> tenantIdMarshaller,
    ...
    public byte[] toRowKeyBytes(T tenantId, R rowKey) throws RowColumnValueStoreMarshallerException {
        try {
            ...
            return join(new byte[]{(byte) tenantIdBytes.length}, tenantIdBytes, rawKey);
        } catch (Exception e) {
    ...
    public TenantIdAndRow<T, R> fromRowKeyBytes(byte[] rowKeyBytes) throws RowColumnValueStoreMarshallerException {
        try {
            int tenantIdSize = rowKeyBytes[0];
            byte[] tenantIdBytes = Arrays.copyOfRange(rowKeyBytes, 1, tenantIdSize);

            T tenantId = tenantIdMarshaller.fromLexBytes(tenantIdBytes);

            byte[] tenantIdRemovedRowKeyBytes = Arrays.copyOfRange(rowKeyBytes, tenantIdSize + 1, rowKeyBytes.length);
